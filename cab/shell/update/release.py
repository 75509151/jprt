...
from update_info import REMOTE_UPDATE_BASE_DIR
from update_utils import (do_cmd, get_recursive_file_list,
                          get_home_dir, md5, cp_folder,
                          change_working_path,
                          generate_md5_info, init_log)
import httplib
import urllib
...
def _get_project_version(svn_path):
    def get_svn_tag_version():
        return_code, out, err = do_cmd("svn info %s" % svn_path)
        try:
            lines = out.split('\n')
            if len(lines) < 4:
...
def release_project(project_path, output_path):
    ...
    cp_folder(project_path, output_path)
    print "cp project to: %s" % output_path

    dosome_folder(parent_path=output_path)
    files = get_recursive_file_list(output_path)
    dosome_files(files)
...
def backup_kiosk_path():
    kiosk_path = os.path.join(get_home_dir(), "kiosk")
    backup_path = os.path.join(get_home_dir(), "kiosk_backup")
    if kiosk_path:
        cp_folder(kiosk_path, backup_path)
...
def recover_kiosk_path():
    ...
    if os.path.exists(backup_path):
        cp_folder(backup_path, kiosk_path)
...
@click.option("--project_path", "-pp", prompt=u"tag所在路径", default=os.path.join(get_home_dir(), "release"), help=u"tag所在路径")
...
def generate_version_to_download(user, pwd, project_path, version, server, after_day):
    ...
    backup_kiosk_path()
    try:
        release_path = os.path.join(get_home_dir(), "kiosk")
        ...
        change_working_path(project_path)
        ...
        tmp_release_path = os.path.abspath(
            os.path.join(get_home_dir(), "tmp_release"))
        ...
        generate_md5_info(project_dest_floder, version_floder, "kiosk.json")

        generate_pwd_file(pwd)
        release_to_remote(user, server, version_floder, REMOTE_UPDATE_BASE_DIR)

        version_info = generate_version_file(project_dest_floder, version, after_day)

        release_to_remote(user, server, version_info, REMOTE_UPDATE_BASE_DIR)
    except Exception as e:
